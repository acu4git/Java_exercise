...
public class MyPaineter2 extends JPanel implements ActionListener, MouseListener {
  // 図形モードの定義
  enum Mode {
    LINE,
    RECT,
  ...
  private JButton btnLine, btnRect, btnOval, btnBlack, btnRed, btnGreen, btnClear, btnQuit, btnUndo, btnRedo;
  ...
  private ArrayList<ShapeWithColor> cache = new ArrayList<>();
  ...
  public static void main(String[] args) {
    ...
    f.setSize(500, 700);
    f.setTitle("MyPainter2");

    MyPaineter2 mp = new MyPaineter2();
  ...
  MyPaineter2() {
    ...
    JButton[] btnList = { btnLine, btnRect, btnOval, btnBlack, btnRed, btnGreen, btnUndo, btnRedo, btnClear, btnQuit };
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (action == btnUndo) {
      if (components.isEmpty())
        return;
      ShapeWithColor temp = null;
      System.out.println("--- UNDO ---");
      temp = components.remove(components.size() - 1);
      cache.add(temp);
      drawPanel.repaint();
    }
    ...
    if (action == btnRedo) {
      ...
      temp = cache.remove(cache.size() - 1);
      components.add(temp);
      drawPanel.repaint();
    }
